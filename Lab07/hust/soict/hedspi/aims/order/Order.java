...
//	private static CompactDisc cd = new CompactDisc();
	...
	private Order() {		// Private constructor
		...
		numberOfOrder++;
//		System.out.println("Order instance count: " + numberOfOrder);
	}
...
//	private static final Object choice(int kind)
//	{
//		switch (kind) {
//		case 0:
//			return b;
...
//	}
	...
	//	Method 1
/*
	public int addMedia(Media[] dvdList) {	// Add by a list (array) of items
		if (itemsOrdered.size() >= MAX_NUMBERS_ORDERED) {
		...
		for (int i = 0; i < itemsOrdered.size(); i++) {
			addMedia(dvdList[i]);	// Add each item in dvdList
		}
	...
	public void printOrder() {
		...
		for (int i = 0; i < itemsOrdered.size(); i++) {
			if (itemsOrdered.get(i) != null) {
				System.out.println("DVD - " + itemsOrdered.get(i).getTitle() + " - " + itemsOrdered.get(i).getCategory() + 
